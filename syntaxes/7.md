...
！重要！ **外側のクエリの行を1行ずつ、内側のサブクエリに"持ち込んで"評価するやつ。**

```txt
外側: student の各行
  ├─ id=1 → 内側実行(exam.id=1は存在する?) → true
  ├─ id=2 → 内側実行(exam.id=2は存在する?) → true
  ├─ id=3 → 内側実行(exam.id=3は存在する?) → true
  └─ id=4 → 内側実行(exam.id=4は存在する?) → true
  └─ id=4 → 内側実行(exam.id=4は存在する?) → false
```
